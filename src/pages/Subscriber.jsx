...
const Subscriber = () => {
  ...
  const [options, setOptions] = useState({ types: [], staTypes: [], subTypes: [], provinces: [] });
  ...
  const [isEdit, setIsEdit] = useState(false); // Edit mode in modal
  const [newSub, setNewSub] = useState({ TYPE: '', STA_TYPE: '', SUB_ID: '', SUB_TYPE: '', STA_DATE: '', END_DATE: '', PROVINCE: '', DISTRICT: '', PCK_CODE: '', PCK_DATE: '', PCK_CHARGE: '' }); // For add form
  const [breakdownTab, setBreakdownTab] = useState('province-district'); // Tabs: 'province-district', 'type', 'sta_type', 'sub_type'
  ...
  useEffect(() => {
    const fetchOptions = async () => {
      try {
        const [types, staTypes, subTypes, provinces] = await Promise.all([
          axios.get('http://localhost:5000/api/subscribers/distinct?field=type'),
          axios.get('http://localhost:5000/api/subscribers/distinct?field=sta_type'), // Note: field lowercase in query, uppercase in DB
          axios.get('http://localhost:5000/api/subscribers/distinct?field=sub_type'),
          axios.get('http://localhost:5000/api/subscribers/distinct?field=province')
        ]);
        setOptions({
          types: types.data,
          staTypes: staTypes.data,
  ...
  // Fetch subscribers with filters/pagination
  useEffect(() => {
    const fetchSubs = async () => {
      ...
      try {
        const params = new URLSearchParams({
          ...
          ...(filters.search && { search: filters.search }) // Add search to backend if needed (in getSubscribers, add if(search) filter.SUB_ID = {$regex: search})
        });
  ...
  useEffect(() => {
    const fetchPies = async () => {
      try {
        ...
        const pastelColors = ['#A7C7E7', '#B9D7EA', '#D6EAF8', '#E0F7FA', '#E8F5E9', '#FBE9E7', '#FFECB3', '#FFE0B2']; // Pastel palette
  ...
  const handleAdd = async (e) => {
    ...
    try {
      ...
      setNewSub({ TYPE: '', /* reset all */ }); // Reset form
      // Refetch subs
      setPage(1);
    } catch (err) {
  ...
  const exportCSV = () => {
    const csvData = subscribers.map(sub => ({
      SUB_ID: sub.SUB_ID,
      TYPE: sub.TYPE,
      STA_TYPE: sub.fullStaType, // Use enriched
  ...
  return (
    <div className="p-4 bg-blue-50 min-h-screen"> {/* Light blue bg */}
      ...
      <div className="mb-8 bg-white p-4 rounded shadow">
        ...
        <form onSubmit={handleAdd} className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <input name="TYPE" value={newSub.TYPE} onChange={(e) => setNewSub({ ...newSub, TYPE: e.target.value })} placeholder="Hình thức (TYPE)" className="p-2 border rounded" required />
      ...
      <div className="flex flex-wrap mb-4 gap-4">
        ...
        <select name="staType" value={filters.staType} onChange={handleFilterChange} className="p-2 border rounded">
          ...
          {options.staTypes.map(s => <option key={s} value={s}>{s}</option>)} {/* Use fullStaType if needed */}
        </select>
            ...
            <tbody>
              {subscribers.map(sub => (
                <tr key={sub._id} onClick={() => openDetails(sub)} className="cursor-pointer hover:bg-blue-50">
                  ...
                  <td className="p-2">{sub.PROVINCE}</td>
          ...
          <div className="bg-white p-6 rounded shadow-lg w-3/4 md:w-1/2">
            ...
            {isEdit ? (
              <form onSubmit={(e) => { e.preventDefault(); handleEdit(); }}>
                <input value={selectedSub.TYPE} onChange={(e) => setSelectedSub({ ...selectedSub, TYPE: e.target.value })} className="w-full p-2 border rounded mb-2" />
                {/* Add other inputs similarly for all fields */}
              ...
              <div>
                <p><strong>Mã:</strong> {selectedSub.SUB_ID}</p>
                <p><strong>Hình thức:</strong> {selectedSub.TYPE}</p>
                {/* Display all fields */}
