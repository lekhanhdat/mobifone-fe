...
const Subscriber = () => {
  ...
  const [options, setOptions] = useState({ types: [], staTypes: [], subTypes: [], provinces: [], pckCodes: [] });
  ...
  const [newSub, setNewSub] = useState({ TYPE: '', STA_TYPE: '', SUB_ID: '', SUB_TYPE: '', STA_DATE: '', END_DATE: '', PROVINCE: '', DISTRICT: '', PCK_CODE: '', PCK_DATE: '', PCK_CHARGE: '' });
      ...
      {selectedSub && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
          <div ref={modalRef} className="bg-white p-6 rounded shadow-lg w-3/4 md:w-2/3 overflow-y-auto max-h-[80vh] grid md:grid-cols-2 gap-4">
            ...
            {isEdit ? (
              <form onSubmit={(e) => { e.preventDefault(); handleEdit(); }} className="col-span-2 grid md:grid-cols-2 gap-4">
                <div>
                ...
                <div>
                  ...
                  <select name="TYPE" value={selectedSub.TYPE} onChange={handleSubChange} className="w-full p-2 border rounded mb-2">
                    <option value="">Hình thức (TYPE)</option>
                    {(options.types || []).map(t => <option key={t.value} value={t.value}>{t.label}</option>)}
                  </select>
                </div>
                <div>
                  ...
                  <select name="STA_TYPE" value={selectedSub.STA_TYPE} onChange={handleSubChange} className="w-full p-2 border rounded mb-2">
                    <option value="">Loại hòa mạng (STA_TYPE)</option>
                    {(options.staTypes || []).map(s => <option key={s.value} value={s.value}>{s.label}</option>)}
                  </select>
                </div>
                <div>
                  ...
                  <input type="date" name="STA_DATE" value={selectedSub.STA_DATE ? selectedSub.STA_DATE.slice(0, 10) : ''} onChange={handleSubChange} className="w-full p-2 border rounded mb-2" />
                </div>
                <div>
                  <label>Loại thuê bao (SUB_TYPE):</label>
                  <select name="SUB_TYPE" value={selectedSub.SUB_TYPE} onChange={handleSubChange} className="w-full p-2 border rounded mb-2">
                    <option value="">Loại thuê bao (SUB_TYPE)</option>
                    {(options.subTypes || []).map(s => <option key={s.value} value={s.value}>{s.label}</option>)}
                  </select>
                </div>
                <div>
                  ...
                  <input type="date" name="END_DATE" value={selectedSub.END_DATE ? selectedSub.END_DATE.slice(0, 10) : ''} onChange={handleSubChange} className="w-full p-2 border rounded mb-2" />
                </div>
                <div>
                  <label>Tỉnh (PROVINCE):</label>
                  <select name="PROVINCE" value={selectedSub.PROVINCE} onChange={handleSubChange} className="w-full p-2 border rounded mb-2">
                    <option value="">Tỉnh (PROVINCE)</option>
                    {(options.provinces || []).map(p => <option key={p.value} value={p.value}>{p.label}</option>)}
                  </select>
                </div>
                <div>
                  ...
                  <select name="DISTRICT" value={selectedSub.DISTRICT} onChange={handleSubChange} className="w-full p-2 border rounded mb-2" disabled={!selectedSub.PROVINCE}>
                    <option value="">Huyện (DISTRICT)</option>
                    {districtOptions.map(d => <option key={d.value} value={d.value}>{d.label}</option>)}
                  </select>
                </div>
                <div>
                  ...
                  <select name="PCK_CODE" value={selectedSub.PCK_CODE} onChange={handleSubChange} className="w-full p-2 border rounded mb-2">
                    <option value="">Mã gói cước (PCK_CODE)</option>
                    {(options.pckCodes || []).map(c => <option key={c.value} value={c.value}>{c.label}</option>)}
                  </select>
                </div>
                <div>
                  ...
                  <input type="date" name="PCK_DATE" value={selectedSub.PCK_DATE ? selectedSub.PCK_DATE.slice(0, 10) : ''} onChange={handleSubChange} className="w-full p-2 border rounded mb-2" />
                </div>
                <div className="col-span-2">
                  <label>Phí (PCK_CHARGE):</label>
                  <input type="number" name="PCK_CHARGE" value={selectedSub.PCK_CHARGE} onChange={handleSubChange} className="w-full p-2 border rounded mb-2" />
                </div>
              ...
              <div className="col-span-2 grid md:grid-cols-2 gap-4">
                ...
                <p><strong>Phí:</strong> {selectedSub.PCK_CHARGE}</p>
